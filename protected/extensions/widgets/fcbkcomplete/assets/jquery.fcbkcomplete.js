...
var dinamycOptions = {json_data : null}; 

jQuery(
    function ($) 
    {        
        $.fn.fcbkcompletechangeoptions = function(opt)
        {
            dinamycOptions = $.extend({ json_data: null }, opt);
            return this;
        };
	    ...
	    $.fn.fcbkcomplete = function (opt) 
	    {
    		return this.each(function()
			{
                ...
                {	    
                   ...
                   process = $(document.createElement("ul"));
                   process.attr({"id" : "progressbar", "class" : "process"});
                   process.hide();
                }
                ...
                function addInput(focusme)
                {
                    ...
                    input.attr({"type": "text", "autocomplete":"off","class": "maininput","size": "1"});                    
                            ...
                            if (event.keyCode != 40 && event.keyCode != 38 && etext.length != 0) 
                            {
                                ...
                                if (options.json_url) 
                                {
                                    ...
                                    {			
                                        ...
                                        if(dinamycOptions.json_data && options.usesdinamyc)                                        
                                            options.json_data = dinamycOptions.json_data;
                                            
                                        if(options.json_data)
                                        {
                                            _json_data = options.json_data;
                                            _json_data[options.searchpropname || 'seatchvalue'] = etext;
                                            for(var key in _json_data) {
                                                if(typeof(_json_data[key]) == 'string')
                                                    _json_data[key].replace(/%s/g, etext);
                                            }
                                        }
                                        else
                                        {
                                            _json_data = {seatchvalue : etext};
                                        }
                                        
                                        $.getJSON(options.json_url, options.json_data, 
                                            function(data)
                                            {
                ...
                function addMembers(etext, data)
                {
                    ...
                    var content = '';
                    var contentX = ''; //temporary placeholder for content
                    var temp = new Array();
                    while (match != null && maximum > 0) 
                    {
                        ...
                        {
                            contentX = object.value + "_" + itemIllumination(object.caption, etext);
                            if (!in_array(contentX, temp)) {
                                temp.push(contentX);
                ...
                {
                    if(func){
                        /*var _object = jQuery.extend({}, item, { notinuse: 0 });
                        */try {
                            func(item);
                        }catch(ex){};
                    }
